...
def main():
	...
	screen = pygame.display.set_mode((windows_width, windows_height))#设置屏幕大小
	screen.fill(white)
...
def running_game(screen,snake_speed_clock):
	startx = random.randint(3, map_width - 8) #随机开始位置
	starty = random.randint(3, map_height - 8)
	snake_coords = [{'x': startx, 'y': starty},  #初始化贪吃蛇
                  {'x': startx - 1, 'y': starty},
                  {'x': startx - 2, 'y': starty}]
	...
	while True:
		...
		snake_speed_clock.tick(snake_speed) #控制帧频
#将食物画出来
...
def show_gameover_info(screen):
	...
	screen.blit(gamestart, (85, 20))
	screen.blit(tip, (80, 500))
	pygame.display.update()
